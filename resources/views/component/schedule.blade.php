...
async function renderAssignTable() {
    // Load current schedules for all classrooms
    ...
    for (const c of classrooms) {
        ...
        try {
            ...
            // Pick the first weekday schedule as representative
        ...
        const opts = shifts.map(s => `<option value="${s.id}" ${s.id === currentShiftId ? 'selected' : ''}>${s.name} (${s.start_time?.substring(0,5)}-${s.end_time?.substring(0,5)})</option>`).join('');
